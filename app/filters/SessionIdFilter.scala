...
//
//          val updatedSession = Session.decodeFromCookie(cookies.get(sessionCookieBaker.COOKIE_NAME)).data
//            .foldLeft(session) {
//              case (m, n) =>
//                m + n
//            }
//
//          result.withSession(updatedSession + (SessionKeys.sessionId -> sessionId))
//          result
//      }
    } else {
      f(rh)
