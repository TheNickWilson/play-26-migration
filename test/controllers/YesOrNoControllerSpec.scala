...
class YesOrNoControllerSpec extends ControllerSpecBase {
  ...
  "YesOrNo Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, yesOrNoRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[YesOrNoView]
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userData = UserData(userDataId, Json.obj(YesOrNoPage.toString -> JsBoolean(true)))
      ...
      contentAsString(result) mustEqual
        view(frontendAppConfig, form.fill(true), NormalMode)(fakeRequest, messages).toString
    }
